...
export const DirectMessagesView: React.FC<DirectMessagesViewProps> = ({
  ...
  const filteredThreads = threads.filter(t => {
    // Category filter
    if (categoryFilter === 'ai' && !(t.threadType === 'ai' || t.id === 'thread-ai-companion' || t.peerRole === 'ai_lantern')) return false;
    if (categoryFilter === 'letter_author' && !(t.threadType === 'letter_author' || Boolean(t.relatedPostTitle))) return false;
    if (categoryFilter === 'peer_listener' && !(t.threadType === 'peer_listener' || t.threadType === 'expert' || t.peerRole === 'peer_listener' || t.peerRole === 'expert')) return false;
    ...
    if (searchQuery.trim()) {
      ...
      return matchName || matchTitle || matchSchool;
    }
          ...
          <div className={`flex-1 min-w-0 flex flex-col h-full bg-[var(--bg-card)] overflow-hidden box-border ${
            ...
            <div className="p-2.5 sm:p-3.5 border-b border-[#E5E2D9] dark:border-[#3A4738] flex items-center justify-between gap-2 min-w-0 shrink-0 relative">
              ...
              <div className="shrink-0 flex items-center gap-1.5">
                ...
                <span className="text-[10px] bg-[#8BA888]/15 text-[#3A5238] dark:text-[#8BA888] border border-[#8BA888]/30 px-2 py-0.5 rounded-full font-bold">
